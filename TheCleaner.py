...
Production-shaped WoodPile Standard pipeline:
...
class App(tk.Tk):
    ...
    def build_video_options(
        ...
        """Return the complete WoodPile Standard video option block."""
    ...
    def build_audio_options(source: SourceInfo) -> list[str]:
        """Return the complete WoodPile Standard audio option block."""
        return [
            "-c:a", "aac",
            "-ac", "2",
    ...
    def build_video_filter(self, scaler: str) -> str:
        """
        Return the complete WoodPile Standard video filter chain.
    ...
    def write_header(
        ...
        self.log_message("\n" + "#" * 78, logfile)
        self.log_message(
            f"The Cleaner v{APP_VERSION} run\n"
